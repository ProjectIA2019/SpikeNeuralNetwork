...
int BOTTON_SPIKE = 2;     //Bottone settaggio modalità di spike [Tonic Spiking , Phasic Spiking , Tonic Bursting , Phasic Bursting , ...]
...
int LED_MOD = 5;          //LED modalità eccitatoria/inibitoria --> Identifica se la modalità inserita è inibitoria o eccitatoria
...
void loop()
{
  digitalWrite(SPIKE,LOW); //Viene ripristinato a 0 il segnale di avviso-spike per il neurone successivo
  if(digitalRead(BOTTON_POT) == HIGH)
  {
    //Resetto i valori di potenziale di membrana e recupero di mambrana
    v = -70;
    u = -20;
...
//--------------------------------------------------------------
  ...
  OutputStr += v; //Linea rossa oscilloscopio
  OutputStr += ", ";
  
  if(pot_disable == 0)
  {
    //Potenziometro attivo
    OutputStr += value; //Linea verde oscilloscopio
  }
  else
  {
    //Potenziometro non attivo
    OutputStr += i  ; //Linea verde oscilloscopio
  }
  ...
  if(v > 30)
  {
    digitalWrite(SPIKE,HIGH); //Viene inviato il segnale al neurone successivo
